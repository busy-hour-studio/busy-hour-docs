...
To listen to all latest activity feeds, you can use the `useLastFeedListener` hook which is exported from the `@busy-hour/react-native/hooks` package. The hook then will return an object detailed as below:
