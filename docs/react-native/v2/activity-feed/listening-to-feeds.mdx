...
To listen to all latest activity feeds, you can use the `useLastFeedListener` hook which is exported from the `@busy-hour/react-native/hooks` or `@busy-hour/react-native/hooks/v2` package. The hook will return an object that can be use to track the latest `feeds`. For more detail about the properties see the detail from this table:
