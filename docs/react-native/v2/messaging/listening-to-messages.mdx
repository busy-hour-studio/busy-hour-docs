...
To listen to all latest messages (e.g. `group only`, `private only`, or `both`), you can use the `useLastMessageListener` hook which is exported from the `@busy-hour/react-native/hooks` or `@busy-hour/react-native/hooks/v2` package. The hook will return an object that contains `chats`, `listenLastMessages`, and `removeLastMessageListener`. For more detail about the properties see the detail from this table:
...
```tsx title="src/main.tsx"
import React, { useEffect } from 'react';
import { ChatType } from '@busy-hour/react-native/enums/v2';
import { useLastMessageListener } from '@busy-hour/react-native/hooks';
