...
To start managing your chat messages (e.g. `create`, `update`, `delete`, etc.), you will need to use the `useChatManager` hook which is exported from the `@busy-hour/react-native/hooks` or `@busy-hour/react-native/hooks/v2` package. To use it, you just need to call it like any other hook and it will return an object. For more detail about the properties see the detail from this table:
