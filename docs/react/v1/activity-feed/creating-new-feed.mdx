---
sidebar_position: 2
sidebar_label: Creating New Activity Feeds
---

import ActivityFeedPrerequisites from '@site/src/components/docs/shared/prerequisites/react-react-native/ActivityFeed';

# Creating New Activity Feeds

In this section, we will show you how to create new activity feeds from the activity feeds service.

## Prerequisites

<ActivityFeedPrerequisites />

## Create New Activity Feeds

To create a new activity feeds, you can use the `useFeedCreator` hook which is exported from the `@busy-hour/react/hooks/v1` package. The hook will return a function that allowing you to create a new activity feed.

### Quick Example

```tsx title="src/main.tsx"
import { useFeedCreator } from '@busy-hour/react/hooks/v1';

export default function Main() {
  // highlight-next-line
  const createFeed = useFeedCreator()

  return <MainApp />
}
```
