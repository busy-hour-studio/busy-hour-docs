...
To listen to all latest activity feeds, you can use the `useLastFeedListener` hook which is exported from the `@busy-hour/react/hooks/v1` package. The hook will return an object that contains `feeds`, `myFeeds`, `listenLastFeeds`, and `removeLastFeedListener`. For more detail about the properties see the detail from this table:
...
import { useLastFeedListener } from '@busy-hour/react/hooks/v1'
