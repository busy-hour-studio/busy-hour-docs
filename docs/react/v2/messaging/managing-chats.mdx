...
To start managing your chat messages (e.g. `create`, `update`, `delete`, etc.), you will need to use the `useChatManager` hook which is exported from the `@busy-hour/react/hooks` or `@busy-hour/react/hooks/v2` package. The hook then will return an object detailed as below:
